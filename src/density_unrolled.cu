    ...
    const unsigned*  __restrict__ layout)
{
    ...
    for (int jl = 0; jl < nj; ++jl)
    {
        ...
        const unsigned js    = layout[jLeaf];
        const unsigned je    = layout[jLeaf + 1];
        const unsigned nj4   = (je - js) & ~3u;  // largest multiple of 4 <= count

        // 4× unrolled: load 12 coordinates in flight, compute 4 distances.
        for (unsigned j = js; j < js + nj4; j += 4)
        {
            double dx0 = xi - x[j  ], dy0 = yi - y[j  ], dz0 = zi - z[j  ];
            double dx1 = xi - x[j+1], dy1 = yi - y[j+1], dz1 = zi - z[j+1];
            double dx2 = xi - x[j+2], dy2 = yi - y[j+2], dz2 = zi - z[j+2];
            double dx3 = xi - x[j+3], dy3 = yi - y[j+3], dz3 = zi - z[j+3];

            double q0 = (dx0*dx0 + dy0*dy0 + dz0*dz0) * hi_sq_inv;
            double q1 = (dx1*dx1 + dy1*dy1 + dz1*dz1) * hi_sq_inv;
            double q2 = (dx2*dx2 + dy2*dy2 + dz2*dz2) * hi_sq_inv;
            double q3 = (dx3*dx3 + dy3*dy3 + dz3*dz3) * hi_sq_inv;

            if (q0 < sph::radk2) { double qij = sqrt(q0), wij, grwij; sph::m4_kern(qij, wij, grwij); rhoi += wij; gradhi += -qij*grwij - 3.0*wij; }
            if (q1 < sph::radk2) { double qij = sqrt(q1), wij, grwij; sph::m4_kern(qij, wij, grwij); rhoi += wij; gradhi += -qij*grwij - 3.0*wij; }
            if (q2 < sph::radk2) { double qij = sqrt(q2), wij, grwij; sph::m4_kern(qij, wij, grwij); rhoi += wij; gradhi += -qij*grwij - 3.0*wij; }
            if (q3 < sph::radk2) { double qij = sqrt(q3), wij, grwij; sph::m4_kern(qij, wij, grwij); rhoi += wij; gradhi += -qij*grwij - 3.0*wij; }
        }

        // Tail: remaining 0–3 particles (identical to original loop body).
        for (unsigned j = js + nj4; j < je; ++j)
        {
            double dx   = xi - x[j];
            double dy   = yi - y[j];
            double dz   = zi - z[j];
